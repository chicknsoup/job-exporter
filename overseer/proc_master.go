...
//				isgood := false
//				if cmd != nil {
//					mp.debugf("slave monitoring, slavepid=" + strconv.Itoa(mp.slaveCmd.Process.Pid))
//					cmd.Wait()
//					for _, arg := range cmd.Args {
//						if !strings.Contains(arg, "node-sd-slave") {
//
//							isgood = false
//						} else {
//							//mp.debugf("still good!")
//							isgood = true
//						}
//					}
//
...
func (mp *master) fork() error {
	...
	cmd.Args = append(os.Args, "--job-exporter-slave")
	cmd.Stdin = os.Stdin
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	//cmd.Args[len(cmd.Args)]="[node-sd-slave=true]"
	//cmd.Args[0]=cmd.Args[0]+"[node-sd-slave]"
	...
	//mp.debugf("xxx"+strconv.Itoa(cmd.Process.Pid))
	mp.slavePid = cmd.Process.Pid
	return nil
}
...
func (mp *master) getpid() int {
	return mp.masterPid
}

func (mp *master) getSlaveCmd() *exec.Cmd {
	return mp.slaveCmd
}
